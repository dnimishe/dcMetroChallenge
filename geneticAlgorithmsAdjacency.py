...
# Function to calculate total distance for a TSP path using an adjacency matrix
def calculate_total_distance(path, adjacency_matrix):
    return sum(adjacency_matrix[path[i], path[i + 1]] for i in range(len(path) - 1))
...
def fitness(path, adjacency_matrix):
    return 1 / calculate_total_distance(path, adjacency_matrix)
...
# Main genetic algorithm with a fixed starting node
def genetic_algorithm(adjacency_matrix, population_size, mutation_rate, generations, start_node):
    num_nodes = len(adjacency_matrix)
    population = create_population(population_size, num_nodes, start_node)
    
    for generation in range(generations):
        # Evaluate fitness for each individual
        fitnesses = [fitness(individual, adjacency_matrix) for individual in population]
    ...
    best_individual = max(population, key=lambda x: fitness(x, adjacency_matrix))
    best_distance = calculate_total_distance(best_individual, adjacency_matrix)
    return best_individual, best_distance

# Example Usage
# Load your adjacency matrix here (should be a 98x98 NumPy array)
e = ExcelFinder()

# adjacency_matrix = np.loadtxt('adjacency_matrix.txt')  # Example file loading
# adjacency_matrix = np.array(e.getAdjacencyMatrixWithInfinity())
# distance_matrix = genfromtxt('distanceMatrix.csv', delimiter=',')
adjacency_matrix = genfromtxt('distanceMatrix.csv', delimiter=',')

# Set parameters
population_size = 500
mutation_rate = 0.01
generations = 500
start_node = 4  # Specify your starting node (e.g., node 0)

# Run the genetic algorithm
best_path, best_distance = genetic_algorithm(adjacency_matrix, population_size, mutation_rate, generations, start_node)
